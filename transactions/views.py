...
class SendMoneyView(TransactionCreateMixin):
    ...
    def form_valid(self, form):
        ...
        if str(receiver) == str(sender_account.account_no):
            form.add_error("receiver_account", "Sorry! you typed your account number")
            return self.form_invalid(form)

        is_bankrupt = BankStatus.objects.order_by("-id").first().is_bankrupt
        if is_bankrupt:
            form.add_error(None, "The account is currently bankrupt.")
            return self.form_invalid(form)
